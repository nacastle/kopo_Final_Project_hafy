...
<<<<<<< HEAD
			    		...
			    		<c:otherwise>
			    		<div id="oriEndPriceTag" style="display: inline-block;">
...
<<<<<<< HEAD
	     ...
	     <div id="priceTag" style="display: none;">
	     	<div style="text-decoration: underline;"><div>최고입찰가</div></div>
			<div style="font-size: 1.5rem; font-weight: bold;">&nbsp;<span id="curPrice"></span> 원 (<span id="bidderNick"></span>) </div>
		</div>
<!-- 		<div id="endPriceTag" style="display: none;"> -->
<!-- 			<div style="text-decoration: underline;"><div>낙찰가</div></div> -->
<!-- 			<div style="font-size: 1.5rem; font-weight: bold; background: rgb(26, 188, 156); display: inline-block;">&nbsp;<span id="endPrice"></span> 원 (<span id="winnerNick"></span>)&nbsp;</div> -->
<!-- 		</div> -->
...
<%--        입찰내역된 적이 없으면 --%>
<%-- 	     	<c:when test="${aucGoodsVO.winningBid == 0 }"> --%>
<!-- 	     		<div class="rank" style="text-align: center;margin-top: 1rem;"> -->
...
<!-- 	     		</div> -->
<!-- 	     		<div class="history" style="display: none;text-align: center;margin-top: 1rem;"> -->
<%-- 	     		 	<img style="width: 6rem; margin-top: 6%; margin-bottom: .5rem; " src="${pageContext.request.contextPath}/resources/img/hafy.png"> --%>
<!-- 	     			<div>아직 입찰 내역이 없습니다.</div> -->
<!-- 	     			<div>해당 경매에 한번 입찰을 해보세요!</div> -->
<!-- 	     		</div> -->
...
<<<<<<< HEAD
	...
	let endStatus;
	let newBidder;
	function getBidInfoHistory() {
		$.get({
			url: "${pageContext.request.contextPath}/getBidInfoHistory/" + ${aucNo},
			success : function(data) {
				
				let bidInfo = JSON.parse(data)
// 				console.log(bidInfo)
				
				let newBid = bidInfo.winningBid;
				newBidder = bidInfo.bidderNick;
				endStatus = bidInfo.endStatus;
				oriBid = $("#curPrice").text();
				oriBid2 = $("#curPrice2").text();
				rfNewBid = comma(newBid) 
				console.log("newBid " + newBid)
				
				if (newBid != 0 && endStatus == "마감전") {
					...
					if (rfNewBid != oriBid) {	
						$("#oriPriceTag").hide()
						$("#priceTag").hide()
						$("#priceTag").addClass("wow flipInX")
						$("#priceTag").show("wow flipInX")
					} else {
						$("#priceTag").removeClass("wow flipInX")
					}
					
					$("#priceTag").show()
					$("#curPrice").text(rfNewBid)
					$("#bidderNick").text(newBidder)
// 					console.log(newBidder)
					
				}
				
				
				if (rfNewBid != oriBid && rfNewBid != oriBid2) {
					console.log("rfNewBid " + rfNewBid)
					console.log("oriBid " + oriBid )
					console.log("oriBid2 " + oriBid2 )
		    		console.log("업데이트")
// 		    		$(".rank").hide()
// 		    		$("#rankTb").show()
		    		getBidInfoHistory2()
		    		loadBidHistory2()
					$("#historyTbody").hide()
					$("#historyTbody").show("wow fadeInUp")
				}
				}
	...
	function getBidInfoHistory2() {
		$.get({
			url: "${pageContext.request.contextPath}/getBidInfoHistory2/" + ${aucNo},
			success : function(data) {
				
				$("#bidderListBody").html(data)
				$("#bidderListBody").hide()
				$("#bidderListBody").show("wow flipInX")
// 				$("#bidderListBody").show()
			},
			error : function() {
				console.log("경매참여자 정보 불러오기 실패")
			}
		})
...
//     console.log($("#oriEndPriceTag").css("display") != "none")
//     console.log($("#oriEndPriceTag").css("display") != "inline-block")
...
//     	else {
//     		if ($("#oriEndPriceTag").css("display") == "none") {
				
//     		$("#endPriceTag").show("wow flipInX")
//     		$("#endPrice").text(rfNewBid)
//     		$("#winnerNick").text(newBidder)
//     		}
...
// 		})
// 	}
	function loadBidHistory2() {
		
		$.ajax({
			url : "${pageContext.request.contextPath}/loadBidHistory2/" + ${aucNo},
			type : 'get',
			success: function(data) {
				
				$("#historyTbody").html(data)
