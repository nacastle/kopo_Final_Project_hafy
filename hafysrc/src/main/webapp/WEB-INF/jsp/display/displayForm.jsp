...
<!--                 <label for="lastName">상품명</label> -->
                <input type="text" class="form-control" name="name" id="name" placeholder="상품명" value="" required autocomplete="off">
                <div class="invalid-feedback">
...
<<<<<<< HEAD
			...
			<div id="startPriceNeed" style="display:none;color: red;">시작가를 입력해주세요.</div>
			
			<hr>
			
			<input id="startDate" type="datetime-local" onchange="setMindate()" class="form-control" name="startDate" style="margin-bottom: 0.5rem;" placeholder="경매 시작일시" required="required">
			<div id="startDateNeed" style="display:none;color: red;">경매 시작일시를 정해주세요.</div>
			<input id="endDate" type="datetime-local" class="form-control" name="endDate" style="margin-bottom: 0.5rem;margin-top: 0.5rem;" placeholder="경매 마감일시" required="required">
			<div id="endDateNeed" style="display:none;color: red;">경매 마감일시를 정해주세요.</div>
            <hr>
...
// 	console.log(new Date().toLocaleString().slice(16,21))
// 	console.log(new Date().toISOString())
// 	console.log(new Date().toISOString().slice(0, 11))
// 	console.log(new Date().toISOString().slice(0, 11) + new Date().toLocaleString().slice(16,21))
...
function openSubmitModal() {
	...
	var photos = f.photos.value;
	var name = f.name.value;
	var category = f.category.value;
	var startPrice = f.startPrice.value;
	var startDate = f.startDate.value;
	var endDate = f.endDate.value;
	var detail = f.detail.value;
	
	
	if (photos == '' || name == '' || category == '' || startPrice == '' || startDate == '' || endDate == '' || detail == '' )  {
		console.log(photos)
		console.log(name)
