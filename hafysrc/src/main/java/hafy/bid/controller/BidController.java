...
//	}
	...
	@ResponseBody
	@GetMapping("/loadBidHistory2/{aucNo}")
	public ModelAndView loadBidHistory(@PathVariable("aucNo") int aucNo) {
	...
	@ResponseBody
	@GetMapping("/getBidInfoHistory/{aucNo}")
	public Map<String, Object> getBidInfoHistory(@PathVariable("aucNo") int aucNo) {
		...
		System.out.println("aucGoodsVO.getRefundStatus()" + aucGoodsVO.getRefundStatus());
		
		if (aucGoodsVO.getRefundStatus().equals("환급완료")) {
			endStatus = "마감";
		} else {
			endStatus = "마감전";
		}
		...
		if (bidResult.size() != 0) {
			bidInfo.put("winningBid", bidResult.get(0).getMemberBalance());
			bidInfo.put("bidderNick", bidResult.get(0).getTranzMemberNick());
		} else {
			bidInfo.put("winningBid", 0);
			bidInfo.put("bidderNick", 0);
		}
		
		bidInfo.put("endStatus", endStatus);
		return bidInfo;
	}

	@ResponseBody
	@GetMapping("/getBidInfoHistory2/{aucNo}")
	public ModelAndView getBidInfoHistory2(@PathVariable("aucNo") int aucNo) {
...
<<<<<<< HEAD
	...
	public int getHighestBid(@PathVariable("aucNo") int aucNo) {
		AucGoodsVO aucGoodsVO = aucGoodsService.selectAucGoodsByNo(aucNo);
		int highestBid = aucGoodsVO.getWinningBid();
