...
<<<<<<< HEAD
	...
	@ResponseBody
	@GetMapping("/getNotiCnt")
	public int getNotiCnt(HttpSession session) {
...
<<<<<<< HEAD
	...
	public Map<String, Object> getBidderCnt(@PathVariable("aucNo") int aucNo, HttpServletRequest request) {
		...
		List<AAccountVO> bidderList = new ArrayList<AAccountVO>();
		bidderList = bidService.selectAAccount(aucNo);
		int bidderCnt = bidderList.size();
		int winningBid = aucGoodsService.selectWinningBidByNo(aucNo);
		
		Map<String, Object> bidInfo = new HashMap<String, Object>();
		bidInfo.put("bidderCnt",bidderCnt);
		bidInfo.put("winningBid",winningBid);
